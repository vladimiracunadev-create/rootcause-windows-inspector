...
**v0.19.0** — sobre todo lo anterior, un nuevo tab **Red** que lleva el enfoque de "distorsión anómala = indicio" al **segmento de red local**: muestra los equipos cercanos (vecinos ARP/NDP, estilo *nmap* honesto) con su IP, MAC, fabricante aproximado (OUI) y rol (tú / router), y aplica el **motor de baseline** ya existente para vigilar la **red conocida** — cualquier equipo nuevo se marca y genera la alerta `unknown-device` (crítica si cambia la MAC de la puerta de enlace, huella típica de suplantación de router). El escaneo pasivo va en cada captura (protección permanente) y hay un **escaneo profundo** bajo demanda (barrido de descubrimiento del `/24` + resolución de nombres). Todo con el encuadre honesto de siempre: descubre *quién está cerca* y avisa de lo no reconocido, **sin** inspeccionar el tráfico de terceros (eso sería un IDS/NDR); complementa —no reemplaza— al firewall. Nuevo módulo `services/netscan.rs`, CLI `rootcause network [--deep] [--json] [--accept]` y documentación (Manual interno + `DETECCION_AMENAZAS.md`, ahora 17 señales).
