...
$iss = Join-Path $root 'packaging\windows\RootCause-Demo.iss'
...
Write-Host "==> Compilando instalador DEMO con $iscc" -ForegroundColor Cyan
& $iscc $iss

$installerDir = Join-Path $root 'build\installer'
if (Test-Path $installerDir) {
    Write-Host "Instalador DEMO generado en: $installerDir" -ForegroundColor Green
}
else {
